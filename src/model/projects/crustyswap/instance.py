...
class CrustySwap:
    def __init__(
        ...
        self.eth_web3 = None
        self.camp_contract = self.camp_web3.web3.eth.contract(address=DESTINATION_CONTRACT_ADDRESS, abi=CRUSTY_SWAP_ABI)
    ...
    async def get_camp_balance(self) -> float:
        """Get native camp balance."""
        try:
            balance_wei = await self.camp_web3.web3.eth.get_balance(self.wallet.address)
            return float(self.camp_web3.web3.from_wei(balance_wei, 'ether'))
        except Exception as e:
            logger.error(f"[{self.account_index}] Failed to get camp balance: {str(e)}")
            return 0
    ...
    async def wait_for_balance_increase(self, initial_balance: float) -> bool:
        """Wait for camp balance to increase after refuel."""
        ...
        while asyncio.get_event_loop().time() - start_time < timeout:
            current_balance = await self.get_camp_balance()
            if current_balance > initial_balance:
                logger.success(
                    f"[{self.account_index}] Balance increased from {initial_balance:.9f} to {current_balance:.9f} camp"
                )
                return True
    ...
    async def refuel(self) -> bool:
        """Refuel camp from one of the supported networks."""
        try:
            await self.initialize()
            # Get current camp balance before refuel
            initial_balance = await self.get_camp_balance()
            logger.info(f"[{self.account_index}] Initial camp balance: {initial_balance:.9f}")
            ...
            has_enough_camp = await self.check_available_camp(amount_wei, contract)
            if not has_enough_camp:
                logger.error(f"[{self.account_index}] Not enough camp in the contract for your amount of ETH deposit, try again later")
                return False
    ...
    async def check_available_camp(self, eth_amount_wei, contract, max_retries=5, retry_delay=5) -> bool:
        """
        Check if there is enough camp in the Crusty Swap contract to fill a buy order.
        ...
        Returns:
            bool: True if there is enough camp, False otherwise
        """
        for attempt in range(1, max_retries + 1):
            try:
                # Get available camp in the contract
                available_camp_wei = await self.camp_web3.web3.eth.get_balance(DESTINATION_CONTRACT_ADDRESS)
                ...
                # Get camp price from contract (in USD with 8 decimals)
                camp_price_usd = await contract.functions.getChainPricePerETH(DESTINATION_CHAIN_ID).call()
                
                # Calculate how many camp we should receive for our ETH
                ...
                # Calculate camp amount from USD value
                camp_price_usd_real = camp_price_usd / 10**8  # Convert to real USD value
                expected_camp_amount = eth_value_usd / camp_price_usd_real
                expected_camp_amount_wei = int(expected_camp_amount * 10**18)
                ...
                logger.info(f"[{self.account_index}] camp price: ${camp_price_usd_real:.4f}")
                logger.info(f"[{self.account_index}] Available camp: {self.camp_web3.web3.from_wei(available_camp_wei, 'ether')} camp")
                logger.info(f"[{self.account_index}] Expected to receive: {expected_camp_amount:.4f} camp")
                
                # Check if there's enough camp in the contract
                has_enough_camp = available_camp_wei >= expected_camp_amount_wei
                
                if has_enough_camp:
                    logger.success(f"[{self.account_index}] Contract has enough camp to fill the order")
                else:
                    logger.warning(f"[{self.account_index}] Contract doesn't have enough camp! " 
                                f"Available: {self.camp_web3.web3.from_wei(available_camp_wei, 'ether')} camp, "
                                f"Needed: {expected_camp_amount:.4f} camp")
                    
                return has_enough_camp
                
            except Exception as e:
                if attempt < max_retries:
                    logger.warning(f"[{self.account_index}] Attempt {attempt}/{max_retries} failed to check available camp: {str(e)}")
                    logger.info(f"[{self.account_index}] Retrying in {retry_delay} seconds...")
                    await asyncio.sleep(retry_delay)
                else:
                    logger.error(f"[{self.account_index}] All {max_retries} attempts failed to check available camp: {str(e)}")
                    return False
        
        # We should never reach here, but just in case
        return False

    async def _get_camp_balance(self, address) -> float:
        """Get native camp  balance for a specific address."""
        try:
            balance_wei = await self.camp_web3.web3.eth.get_balance(address)
            return float(self.camp_web3.web3.from_wei(balance_wei, 'ether'))
        except Exception as e:
            logger.error(f"[{self.account_index}] Failed to get camp balance: {str(e)}")
            return None
    ...
    async def _wait_for_balance_increase(self, initial_balance: float, address: str) -> bool:
        ...
        # Check balance every 5 seconds until timeout
        while asyncio.get_event_loop().time() - start_time < timeout:
            current_balance = await self._get_camp_balance(address)
            if current_balance > initial_balance:
                logger.success(
                    f"[{self.account_index}] Balance increased from {initial_balance} to {current_balance} camp"
                )
                return True
    ...
    async def send_refuel_from_one_to_all(self, address) -> bool:
        ...
        try:
            initial_balance = await self._get_camp_balance(address)
            if initial_balance is None:
                logger.error(f"[{self.account_index}] Failed to get camp balance for address: {address}")
                return False
            logger.info(f"[{self.account_index}] Initial camp balance: {initial_balance}")
            if initial_balance > self.config.CRUSTY_SWAP.MINIMUM_BALANCE_TO_REFUEL:
    ...
    async def refuel_from_one_to_all(self, private_keys_to_distribute) -> bool:
        """Refuel camp from one of the supported networks to multiple addresses."""
        try:
            await self.initialize()
